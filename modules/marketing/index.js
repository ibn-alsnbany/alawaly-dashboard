...
export const marketingModule = {
    render: () => {
        return `
            <div class="mb-8">
                <h1 class="text-3xl font-black text-slate-900 dark:text-white mb-2">${i18n.t('marketing')}</h1>
                <p class="text-slate-500 text-[0.8125rem] font-medium opacity-80">${i18n.t('marketingSubtitle') || 'إدارة الحملات الإعلانية وتحليل قنوات التسويق.'}</p>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 mb-12 w-full">
                ${statCard(i18n.t('campaigns'), `<span class="font-nums">12 Active</span>`, `<span class="font-nums">Stable</span>`, 'vision-blue', 'M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z')}
                ${statCard(i18n.t('leads'), `<span class="font-nums">450</span>`, `<span class="font-nums">+22%</span>`, 'vision-purple', 'M13 10V3L4 14h7v7l9-11h-7z')}
                ${statCard(i18n.t('clicks'), `<span class="font-nums">12,400</span>`, `<span class="font-nums">+5%</span>`, 'vision-orange', 'M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5')}
                ${statCard(i18n.t('roi'), `<span class="font-nums">3.4x</span>`, `<span class="font-nums">+0.2</span>`, 'vision-green', 'M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z')}
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 bg-white dark:bg-vision-surface p-8 rounded-[2rem] border border-slate-100 dark:border-vision-border shadow-soft">
                    <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-8">${i18n.t('performance')}</h3>
                ...
                <div class="bg-white dark:bg-vision-surface p-8 rounded-[2rem] border border-slate-100 dark:border-vision-border shadow-soft">
                    <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-8">${i18n.t('channels')}</h3>
                    <div class="space-y-7">
                        ...
                        ${channel('Google Search', `<span class="font-nums">25%</span>`, 'bg-vision-blue')}
