            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-1 space-y-6">
                    <div class="premium-card !p-6 bg-gradient-to-br from-indigo-600 to-blue-700 border-none text-white">
                        <h3 class="text-lg font-bold mb-6">${i18n.t('hrSummary')}</h3>
                        <div class="space-y-4">
                            <div class="flex justify-between items-center opacity-90">
                                <span class="text-sm font-medium">${i18n.t('avgPerformance')}</span>
                                <span class="font-nums font-bold">92%</span>
                            </div>
                            <div class="flex justify-between items-center opacity-90">
                                <span class="text-sm font-medium">${i18n.t('attendanceRate')}</span>
                                <span class="font-nums font-bold">94%</span>
                            </div>
                            <div class="pt-4 border-t border-white/10 flex justify-between items-center">
                                <span class="text-sm font-bold">${i18n.t('totalSalaries')}</span>
                                <span class="text-lg font-bold font-nums">${formatCurrency('480,000')}</span>
                            </div>
                        </div>
                    </div>

                    <div class="premium-card !p-6">
                        <h3 class="text-[0.9375rem] font-bold text-slate-700 dark:text-slate-200 mb-6">${i18n.t('performanceOverview')}</h3>
                        <div class="space-y-5">
                            ${performanceBar(i18n.t('techManager') || 'Tech Manager', 95, 'bg-indigo-500')}
                            ${performanceBar(i18n.t('uiDesigner') || 'UI Designer', 88, 'bg-emerald-500')}
                            ${performanceBar(i18n.t('dataAnalyst') || 'Data Analyst', 75, 'bg-amber-500')}
                        </div>
                    </div>
                </div>
            </div>
